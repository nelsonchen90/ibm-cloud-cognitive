...
import { Activity16, Restart16, Download16, Filter16 } from '@carbon/icons-react';
import { DataTable, Button, Pagination } from 'carbon-components-react';
import { Datagrid, useDatagrid, useInfiniteScroll, useNestedRows,
  useExpandedRow, useRowIsMouseOver,
  useSelectRows, useOnRowClick, useSortableColumns, useColumnRightAlign, useDisableSelectRows,
  useCustomizeColumns,
  useSelectAllWithToggle } from '.';
  import { /*StickyActionsColumn,*/ CustomizeColumnStory, RowSizeDropdownStory, SelectAllWitHToggle, LeftPanelStory } from './Datagrid.stories';
...
export const NestedTable = () => {
  ...
  const expansionRenderer = () => (
    <div className="carbon-nested-table"><Datagrid {...nestedDatagridState} /></div>
  );
...
const DatagridActions = (datagridState) => {
  ...
  return isNothingSelected &&
    <React.Fragment>
      ...
      />
      <TableToolbarContent>
        <TableToolbarSearch size="xl" id="columnSearch" persistent placeHolderText={searchForAColumn} onChange={(e) => setGlobalFilter(e.target.value)} />
        <RowSizeDropdown
          {...rowSizeDropdownProps}
        />
...
export const CustomizingColumns = () => {
  ...
  return (
    <>
      ...
      <p>More details in the <strong>Notes</strong> section</p>
    </>
  );
};
