...
import { Modal, PropTypes } from 'carbon-components-react';
...
const CustomizeColumnsModal = ({
  ...
  const onRequestSubmit = () => {
    setIsModalOpen(false);
    const updatedColumns = columnObjects
      .map((colDef) => ({
        id: colDef.id,
  ...
  return (
    <Modal
      className="datagrid-customize-columns-modal"
      open={isOpen}
      ...
      selectorPrimaryFocus=".datagrid-customize-columns-column-list--focus"
      ...
      hasForm
    >
      <div className="datagrid-customize-columns-instructions">
        { instructionsLabel }
      </div>
      <Actions
