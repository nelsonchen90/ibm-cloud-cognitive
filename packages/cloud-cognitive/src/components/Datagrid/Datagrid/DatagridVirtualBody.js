  ...
  return (
    <TableBody {...getTableBodyProps()} onScroll={onScroll}>
      <VariableSizeList
        ...
        itemSize={(index) => visibleRows[index].isExpanded ? (visibleRows[index].expandedContentHeight || 0) + rowHeight : rowHeight}
        estimatedItemSize={rowHeight}
        onScroll={onScroll}
        innerRef={innerListRef}
        ref={listRef}
        className="datagrid-virtual-scrollbar"
      >
        {({ index, style }) => {
          const row = visibleRows[index];
          prepareRow(row);
          return (
            <div style={{ ...style, overflow: withOverflowRow ? 'visible' : 'hidden' }}>
