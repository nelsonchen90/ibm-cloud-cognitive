...
const styleClassPrefix = 'datagrid-right-sticky-column';
...
const useStickyColumn = (hooks) => {
  ...
  const useEventListener = (instance) => {
    useEffect(() => {
      ...
      const boundListener = debounce(onBodyResize.bind(null, tableBodyElement, headerCellElement), 250);
...
const toggleStickyShadow = (tableBodyEle, headerCellEle) => {
  if (tableBodyEle && headerCellEle) {
    const isScrolledToRight = tableBodyEle.scrollLeft + tableBodyEle.clientWidth === tableBodyEle.scrollWidth;
    if (isScrolledToRight) {
      headerCellEle.classList.add('sticky-noShadow');
      tableBodyEle.classList.add('sticky-column-noShadow');
    } else {
      headerCellEle.classList.remove('sticky-noShadow');
      tableBodyEle.classList.remove('sticky-column-noShadow');
    }
  }
};
