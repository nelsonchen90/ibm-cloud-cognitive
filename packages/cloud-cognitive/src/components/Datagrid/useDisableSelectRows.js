...
const updateSelectAll = (hooks) => {
  const getToggleAllRowsSelectedProps = (props, { instance }) => {
    const selectableRows = instance.rows.filter(
      (row) => !(instance.shouldDisableSelectRow && instance.shouldDisableSelectRow(row))
    );
    const isAllRowsSelected = selectableRows.length > 0 && selectableRows.every(({ id }) => instance.state.selectedRowIds[id]);
    return [
      props,
      {
        onChange: (e) => {
          selectableRows.forEach((row) => row.toggleRowSelected(e.target.checked));
        },
