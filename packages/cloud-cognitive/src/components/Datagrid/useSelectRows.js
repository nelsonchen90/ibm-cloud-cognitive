...
const useHighlightSelection = (hooks) => {
  const getRowProps = (props, { row }) => [
    props,
    { className: cx('datagrid-carbon-row', row.getToggleRowSelectedProps().checked ? 'bx--data-table--selected' : '') },
  ];
...
const SelectRow = (datagridState) => {
  const {
    isFetching = false, tableId, row, cell, radio, toggleAllRowsSelected, onRadioSelect,
  } = datagridState;
  ...
  return (
    <TableSelectRow
      ...
      className="datagrid-checkbox-cell"
